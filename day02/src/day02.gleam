...
pub fn main() {
  ...
  let the_main_memory: lib.Memory = lib.make_this_memory(memory_as_csv_string)
  let initial_state_a =
    lib.IntCode(
      pointer: 0,
  ...
  // Part B
  let pairs: List(#(Int, Int)) = lib.make_candidate_pairs()
  let #(#(noun, verb), _) = lib.find_winner(pairs, the_main_memory)
  let answer_2 = {
    { 100 * noun } + verb
  }
